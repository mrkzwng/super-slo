...
rm -f ${PREFIX}_frame[123456789].tmp.txt ${PREFIX}_delayed1_*.png ${PREFIX}_delayed2_*.png

ffmpeg -loglevel error -i $FILE -vf scale=352:352 ${PREFIX}_%04d.png
...
for X in `seq 1 $(( NUM_FRAMES - 8 ))`; do
    ...
    ln -s $( basename $( printf "%s_%04d.png" $PREFIX $(( X + 3 )) ) ) $( printf "%s_delayed2_%04d.png" $PREFIX $X )
    ln -s $( basename $( printf "%s_%04d.png" $PREFIX $(( X + 4 )) ) ) $( printf "%s_delayed2_%04d.png" $PREFIX $X )
    ln -s $( basename $( printf "%s_%04d.png" $PREFIX $(( X + 5 )) ) ) $( printf "%s_delayed2_%04d.png" $PREFIX $X )
    ln -s $( basename $( printf "%s_%04d.png" $PREFIX $(( X + 6 )) ) ) $( printf "%s_delayed2_%04d.png" $PREFIX $X )
    ln -s $( basename $( printf "%s_%04d.png" $PREFIX $(( X + 7 )) ) ) $( printf "%s_delayed2_%04d.png" $PREFIX $X )
    ln -s $( basename $( printf "%s_%04d.png" $PREFIX $(( X + 8 )) ) ) $( printf "%s_delayed2_%04d.png" $PREFIX $X )
done    
...
join ${PREFIX}.d1.cut.psnr ${PREFIX}.d2.cut.psnr | sed 's/psnr_avg://g;s/n://;s/\.[0-9]*//g' | while read FRAMENO PSNR1 PSNR2; do
    ...
    if [ "$FRAMENO" -le $(( NUM_FRAMES - 8 )) ] && [ "$PSNR1" != "inf" ] && [ "$PSNR1" -lt 35 ] && [ "$PSNR2" != "inf" ] && [ "$PSNR2" -lt 35 ]; then
...
mv ${PREFIX}_frame4.tmp.txt ${PREFIX}_frame4.txt
mv ${PREFIX}_frame5.tmp.txt ${PREFIX}_frame5.txt
mv ${PREFIX}_frame6.tmp.txt ${PREFIX}_frame6.txt
mv ${PREFIX}_frame7.tmp.txt ${PREFIX}_frame7.txt
mv ${PREFIX}_frame8.tmp.txt ${PREFIX}_frame8.txt
mv ${PREFIX}_frame9.tmp.txt ${PREFIX}_frame9.txt
rm -f ${PREFIX}.d1.psnr
rm -f ${PREFIX}.d2.psnr
