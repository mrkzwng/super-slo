...
class MotionDetector:
    ...
    def trim(self, avi_path):
        ...
        starts = list(range(0, total_frames, int(total_frames / 4)))

        for clip_idx in range(0, 4):
            path = os.path.join(dir_path, str(clip_idx))
            os.mkdir(path)
            frame_idx = 0
            cap.set(int(cv2.CAP_PROP_POS_FRAMES), starts[clip_idx])
            ...
            while self._difference(old_frame, frame) > self.threshold:
                ...
                ### DEBUG
                print(clip_idx + 1, cap.get(cv2.CAP_PROP_FRAME_COUNT), 
